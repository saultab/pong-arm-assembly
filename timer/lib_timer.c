...
uint32_t init_timer ( uint8_t timer_num, uint32_t Prescaler, uint8_t MatchReg, uint8_t SRImatchReg, uint32_t TimerInterval )
{
	...
	else if( timer_num == 2 )
  {
		...
		NVIC_SetPriority(TIMER2_IRQn, 0);	/* less priority than buttons and timer0 e timer1*/
		return (1);
  }
	else{
		...
		NVIC_SetPriority(TIMER3_IRQn, 3);	/* less priority than buttons and timer0, timer1 e timer2*/
		return (1);
	}
