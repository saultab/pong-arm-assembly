...
extern unsigned short pallina_Y, pallina_X, AD_current, AD_last, increment_x, increment_y;
...
void PutChar_Player2( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
	uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer,ASCI);  /* 取字模数据 */
...
void GUI_Text_Player2(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{
		uint8_t TempChar;
		do
			{	
        TempChar = *str++;
        PutChar_Player2( Xpos, Ypos, TempChar, Color, bkColor );    
        if( Xpos < MAX_X - 8 )
        {
            Xpos -= 8;
        } 
...
void paint_ball(int pallina_X, int pallina_Y, uint16_t Color){
	int i;
	for(i=0; i<5; i++)
			LCD_DrawLine(pallina_X - 2, pallina_Y + i,pallina_X + 2, pallina_Y + i, Color);
}
...
void preview(void){
	...
	GUI_Text(20, 120, (uint8_t *) "Premi KEY1", White, Black);
	GUI_Text(20, 140, (uint8_t *) "per iniziare una partita", White, Black);
	GUI_Text(20, 160, (uint8_t *) "Premi KEY2", White, Black);
	GUI_Text(20, 180, (uint8_t *) "per mettere in pausa", White, Black);
}

void start(void){
	/*Stampo Istruzioni*/
	GUI_Text(20, 120, (uint8_t *) "Premi KEY1", Black, Black);
	GUI_Text(20, 140, (uint8_t *) "per iniziare una partita", Black, Black);
	GUI_Text(20, 160, (uint8_t *) "Premi KEY2", Black, Black);
	GUI_Text(20, 180, (uint8_t *) "per mettere in pausa", Black, Black);
	GUI_Text(10, 160, (uint8_t *) score1, White, Black);
	GUI_Text_Player2(226, 176, (uint8_t *) score2, White, Black); //traslato di +16x +16y
}
